...
#
"$w2wprefix"word2vec -size 300 -window 8 -sample 1e-5 -negative 8 -threads 24 -min-count 3 -alpha 0.025 -classes 0 -debug 1 -binary 1 -portable 1 -save-vocab "$resprefix"vocab.txt -cbow 0 -train "$resprefix"docs.word2vec.txt -output "$resprefix"vectors.bin
...
strusInspectVectorStorage -S "$srcprefix"config/vsm.conf featname    | iconv -c -f utf-8 -t utf-8 - | sed -E 's/[\\\>\<\"/]//g' | $scriptdir/createFeatureRules.pl - lexem name '' $pattern_stopwords_doc > $pattern_forwardfeat_doc
