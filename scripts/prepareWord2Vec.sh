...
strusInspect -s "path=storage1" content orig | perl -C -pe 's/^[0-9]+[:]//' > docs.1.txt
strusInspect -s "path=storage2" content orig | perl -C -pe 's/^[0-9]+[:]//' > docs.2.txt
strusInspect -s "path=storage3" content orig | perl -C -pe 's/^[0-9]+[:]//' > docs.3.txt

for dd in 1 2 3
do
cat docs.$dd.txt | iconv -c -f utf-8 -t utf-8 - | perl -C -pe 's/\&[a-z]*\;/ /g' | perl -C -pe 's/nbsp[\;]/ /g' | perl -C -pe 's/[—––_=\/;,()\[\]\{\}\"-\-]/ /g' | perl -C -pe 's/([\!\?\:\.])/ \1 /g' | perl -C -pe 's/[ ]+/ /g' | perl -C -pe 's/[ ][.][ ]/ .\n/g' > docs.stripped.$dd.txt
