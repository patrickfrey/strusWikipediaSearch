...
strusWebService -c build/config/config_stat1.js -V
...
echo "--- Statistics server (global term statistics map) ---"
curl -d "@build/config/statserver.json" -i -H "Accept: text/plain" -H "Content-Type: application/json; charset=UTF-8" -X PUT "$SSERVER1/statserver/statserver"
