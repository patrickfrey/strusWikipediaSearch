...
strusInspect -s "path=storage" ttc tist > resources/metadata_tist_doclen.txt
# [1.2] update the meta data table element doclen_tist with the title lengths calculated
strusUpdateStorage -s "path=storage" -m doclen_tist resources/metadata_tist_doclen.txt


# [2] Initialize the link popularity weight in document meta data (element pageweight):
# [2.1] get the link reference statistics
strusInspect -s "path=storage" fwstats linkid > resources/linkid_list.txt
# [2.2] get the docno -> docid map
strusInspect -s "path=storage" attribute title > resources/docid_list.txt
...
strusUpdateStorage -s "path=storage" -m pageweight resources/pageweight_map.txt
