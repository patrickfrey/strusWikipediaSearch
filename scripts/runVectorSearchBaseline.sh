...
run() {
	...
	strusInspectVectorStorage -t 20 -N 100 -x -s "path=$vsmdir/vsm" opfeatwname \%$feat1 > $resfileBase
	strusInspectVectorStorage -t 20 -N 20     -s "path=$vsmdir/vsm" opfeatwname \%$feat1 > $resfileSearch
}
